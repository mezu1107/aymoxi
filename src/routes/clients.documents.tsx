...
export const Route = createFileRoute("/clients/documents")({
  head: () => ({
    meta: [
      { title: "Documents — AYMOXI Client Portal" },
...
function Documents({ clientId }: { clientId: string }) {
  ...
  return (
    <div>
      ...
      {loading ? <EmptyState label="Loading documents…" /> : rows.length === 0 ? <EmptyState label="No documents shared yet." /> : (
          ...
          {rows.map((d) => (
            <a key={d.id} href={d.url} target="_blank" rel="noreferrer"
              ...
              <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                <span>{[d.file_type, size(d.file_size)].filter(Boolean).join(" · ") || new Date(d.created_at).toLocaleDateString()}</span>
