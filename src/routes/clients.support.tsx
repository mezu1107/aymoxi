...
export const Route = createFileRoute("/clients/support")({
  head: () => ({
    meta: [
      { title: "Support — AYMOXI Client Portal" },
      { name: "description", content: "Raise a support request and track replies from the AYMOXI team." },
  ...
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div>
          {loading ? <EmptyState label="Loading requests…" /> : rows.length === 0 ? <EmptyState label="No support requests yet." /> : (
                ...
                <div key={t.id} className="rounded-2xl border border-border bg-card p-5">
                  ...
                  {t.reply && (
                    <div className="mt-3 rounded-xl border-l-4 border-primary bg-muted/50 p-3">
                      <p className="text-[10px] font-black uppercase tracking-widest text-primary">AYMOXI reply</p>
        ...
        <form onSubmit={submit} className="h-fit rounded-2xl border border-border bg-card p-5">
          ...
          <div className="space-y-3">
            ...
            {sent && <p className="text-xs text-primary">Request submitted — we'll be in touch.</p>}
