...
export const Route = createFileRoute("/team/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    if (!m) {
      return { meta: [{ title: "Team member not found — AYMOXI" }, { name: "robots", content: "noindex" }] };
    }
    const title = `${m.name}${m.role_title ? ` — ${m.role_title}` : ""} | AYMOXI`;
    const description = (m.bio || m.long_bio || `${m.name} at AYMOXI.`).slice(0, 155);
    return {
      meta: [
        { title },
      ...
      <section className="py-12 sm:py-16">
        <div className="mx-auto grid max-w-5xl gap-8 px-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-8">
          ...
          <aside className="min-w-0 space-y-6">
            ...
            <div className="rounded-3xl bg-gradient-to-br from-[#123409] to-[#0a2205] p-6 text-white">
              <p className="font-display text-lg font-black">Work with our team</p>
              <p className="mt-2 text-sm text-white/70">Tell us about your project and we'll match you with the right specialists.</p>
              <Link to="/contact" className="mt-4 inline-flex rounded-full bg-copper px-5 py-2.5 text-xs font-bold text-espresso hover:bg-white">
                Start a project
              </Link>
            </div>
          </aside>
