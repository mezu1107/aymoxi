...
export const Route = createFileRoute("/quote")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Request a Quote — AYMOXI" },
    ...
    <main className="pt-28 pb-20 bg-sand/30 min-h-screen">
      <div className="mx-auto max-w-3xl px-6">
        <div className="text-center">
          <span className="inline-block rounded-full bg-espresso/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-espresso">Request a Quote</span>
          <h1 className="mt-4 font-display text-4xl font-black text-espresso sm:text-5xl">Let's price your project</h1>
          <p className="mt-3 text-foreground/70">Fill in the details — we usually respond within 1 business day.</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="mt-10 rounded-3xl border border-espresso/10 bg-white p-6 shadow-soft sm:p-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Phone" name="phone" placeholder="+1 720 794 1888" />
            <Field label="Company" name="company" placeholder="Acme Inc." />
            <Select label="Service needed" name="service" options={["Web Development","Mobile App","AI / Automation","Cloud & DevOps","UI / UX Design","Digital Marketing","Ecommerce","Other"]} />
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
            <a href={`tel:${PHONE.replace(/\s/g,"")}`} className="inline-flex items-center gap-2 text-sm font-bold text-espresso hover:text-cocoa">
